...
def get_counterspelled_Accel_Synchro(connection: sqlite3.Connection, spell_id: int):
	'''
...
def get_reaction_Accel_Synchro(connection: sqlite3.Connection, spell_id: int):
	'''
...
def get_main_action_Accel_Synchro(connection: sqlite3.Connection, spell_id: int):
	'''
...
def get_Fusion_targets(connection: sqlite3.Connection, spell_id: int):
	'''
	...
	# the strategy is to join Spell, Spell_Fusion, and Fusion together.
	# Next, select fusion_id's in Spell_Fusion based on which spell_id, fusion_id
	# pairs match our input spell_id, then use this result to filter down
	# our original join. Exclude input spell from results.
	gestalt_cursor.execute(
		"""
		SELECT * FROM Fusion
